                ...
                <nav className="breadcrumb__container">
                    ...
                    {crumbPath} {
                        searchParams?.get('category')
                            ?<span className="breadcrumb__container--crumb-span"><Icon icon="material-symbols:chevron-right-rounded" />{ categories?.find((item) => item.id == searchParams?.get('category'))?.name}</span>
                            :searchParams?.get('season')
                            ?<span className="breadcrumb__container--crumb-span"><Icon icon="material-symbols:chevron-right-rounded" />{'mùa '+ seasons?.find((item) => item.id == searchParams?.get('season'))?.name}</span>
                            :``}
                    {/*<span className="breadcrumb__container--crumb-span">{searchParams?.get('category') || searchParams?.get('season') ? `trang ${searchParams?.get('page')}` : ''}</span>*/}
                </nav>
            </Container>
        </div>
    );
